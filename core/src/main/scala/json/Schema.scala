...
object Schema {
	...
	// +---------------
	//
	final case class `array`[T, C[_]](componentType: T/*Schema[T]*/, unique: Boolean = false) extends Schema[C[T]] {
		type Self = `array`[T, C]
	...
	object `object` {
		...
		// NOTE: changed here @statisticallyfit - the tpe: Schema[]T into tpe: T for  fix type usage
		...
		// NOTE: changed here tpe: Schema[T] --> tpe: T
		object Field {
			sealed trait RWMode
			object RWMode {
			...
			def apply[T](name: String, tpe: T/*Schema[T]*/): Field[T] =
				new Field(name, tpe, required = true, default = None, description = None, RWMode.ReadWrite)

			def apply[T](name: String, tpe: T/*: Schema[T]*/, required: Boolean): Field[T] =
	...
	// +---------------
	//
	// NOTE: changed here tpe: Schema[_] into tpe: T
	final case class `enum`[T](tpe: T/*Schema[_]*/, values: Set[Value]) extends Schema[T] {
		type Self = `enum`[T]
